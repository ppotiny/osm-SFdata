...
def top_k(count=50):
    """By default, this prints the 50 most-used keys in node tags"""
...
def top_v(k, count=50):
    """By default, this prints the 50 most-used values for a specific key in node tags"""
...
def audit_phones(phone):
    ...
    phone = ''.join(phone)
    # +1 is in U.S. numbers, add this number if it's not already present
    if phone[0] != '1':
        phone = '1'+phone
    if len(phone) == 11:  # Some phone numbers are missing digits, so check if there are 11
        phone = '+'+phone[0]+'-'+phone[1:4]+'-'+phone[4:7]+'-'+phone[7:]
    if not re.match(good_phone_format, phone):
        # If the phone does not match the proper format, return None
        return None
    else:
        return phone
...
# I had planned on putting cardinal directions as well, but some streets
# contained letters where it was not clear if 'E' meant 'East' or just 'E'.
